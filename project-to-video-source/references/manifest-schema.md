...
```jsonc
{
  ...
  "scenes": [
    {
      ...
      "narration": "…",          // non-empty; fed to NetMind by remotion-video
      "source": {
  ...
  "render": {                    // OPTIONAL — remotion-video fills this in later
    ...
    "narrationAudio": "assets/narration.mp3", // NetMind full track (single audio)
    "captions": true
  }
}
```
...
of that scene's NetMind narration audio, so main-video cuts land exactly where
